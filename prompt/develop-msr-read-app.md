...
3. 사용 할 device path 선택
   ...
   2. list_path 중 끝이 "&msr" 끝나는 device path 를 선택(s_selected_path)
...
8. 장비를 magnetic card reading 대기 상태로 전환
   1. `g_ctl_lpu237.read_card_from_device_with_callback(true,_cb_read_msr_done,_cb_msr_read_error)`
   2. _cb_read_msr_done() 과 _cb_msr_read_error() 의 예시는 "예시 코드"에 있음. _cb_read_msr_done() 가 호출되면, 장비는 자동적으로 카드 읽기 대기 상태로 재진입, _cb_msr_read_error() 가 호출되면, 장비는 자동적으로 카드 읽기 대기 종료됨
9. 장비가 magnetic card reading 대기 상태 중, 대기를 취소하고 싶으면
...
// 카드가 정상으로 읽거나 카드 읽기 에러가 있을 호출되는  
function _cb_read_msr_done( n_device_index:number, s_msg:string ){
    ...
    for( let i = 0; i<3; i++ ){
        ...
        if( g_ctl_lpu237.get_device().get_msr_error_code(i) !== 0 ){
            ...
            // TODO. g_ctl_lpu237.get_device().get_msr_error_code(i) 를 이용해서 에러 코드 표시.
            continue;
        }

        //get a track data.
        let s_card = g_ctl_lpu237.get_device().get_msr_data(i);
        if( s_card.length == 0 ){
            //TODO.  현재 ISO track의 데이터가 없음.
        }
        else{
            //TODO. s_card 에 있는 카드 데이터를 원하는 곳에 표시
        }
    }//end for
    ...
    //자동적으로 카드 읽기 대기 상태로 재진입.
}

// 카드가 읽기 대기 중 에러가 발생한 경우.
function _cb_read_msr_error( n_device_index:number,event_error:Error){
    // TODO. n_device_index 에 의해 지정된 device 에서 읽기 대기 중 event_error 에러 발생 처리.

    //장비는 자동적으로 카드 읽기 대기 종료됨
}

function _cb_stop_msr_done( n_device_index:number,s_msg:string ){
    // TODO. n_device_index 에 의해 지정된 device 의 읽기 대기 상태 취소됨.
    // s_msg 는 항상 "success" 이므로 무시 가능.
}

function _cb_stop_msr_error(n_device_index:number,event_error:Error){
