...
import { AppState, ConnectionStatus, MsrTrackData } from './types';
...
const MSR_DEVICE_FILTER = 'hid#vid_134b&pid_0206&mi_01';
const MSR_PATH_SUFFIX   = '&msr';
...
export function createHandlers(
  ...
  function _cb_read_msr_done(n_device_index: number, s_msg: string):void {
    addLog(`[MSR] Card read done (device[${n_device_index}]): ${s_msg}`);

    if (!g_ctl_lpu237) return;

    const trackData: MsrTrackData = {
      track1: '',
      track2: '',
      track3: '',
      track1_error: 0,
    ...
    for (let i = 0; i < 3; i++) {
      const errCode = g_ctl_lpu237.get_device()!.get_msr_error_code(i);
      if (typeof errCode !== 'number') continue;
      if (errCode !== 0) {
        if (i === 0) trackData.track1_error = errCode;
        else if (i === 1) trackData.track2_error = errCode;
        else trackData.track3_error = errCode;
        addLog(`[MSR] Track${i + 1} error code: ${errCode.toString(16)}h(${errCode.toString(10)})`);
      } else {
        const data = g_ctl_lpu237.get_device()!.get_msr_data(i);
        if (i === 0) trackData.track1 = data;
        else if (i === 1) trackData.track2 = data;
        else trackData.track3 = data;
        if (data.length > 0) {
          //addLog(`[MSR] Track${i + 1}: ${data}`);
        } else {
          //addLog(`[MSR] Track${i + 1}: (empty)`);
        }
      }
    }
    ...
    setState(prev => ({
      ...prev,
      cardHistory: [trackData, ...prev.cardHistory].slice(0, 50),
    }));
    // 자동으로 카드 읽기 대기 재진입 (라이브러리가 처리)
  }

  // ── 카드 읽기 에러 콜백 ──────────────────────────────────── 
  function _cb_read_msr_error(n_device_index: number, event_error: Error | Event):void {
    if( event_error instanceof Error){
      addLog(`[MSR] Read error (device[${n_device_index}]): ${event_error.message}`);
    }
    // 라이브러리가 자동으로 카드 읽기 대기 종료
    setState(prev => ({ ...prev, isReading: false }));
  }

  // ── 카드 읽기 취소 완료 콜백 ───────────────────────────────
  function _cb_stop_msr_done(n_device_index: number, s_msg: string):void {
    addLog(`[MSR] Read cancelled (device[${n_device_index}]): ${s_msg}`);
    setState(prev => ({ ...prev, isReading: false }));
  }

  // ── 카드 읽기 취소 에러 콜백 ───────────────────────────────
  function _cb_stop_msr_error(n_device_index: number, event_error: Error| Event):void {
    if( event_error instanceof Error){
      addLog(`[MSR] Cancel error (device[${n_device_index}]): ${event_error.message}`);
    }
    setState(prev => ({ ...prev, isReading: false }));
  }
  ...
  function _cb_before_unload(): void {
    if (g_ctl_lpu237 && stateRef.current.isReading) {
      g_ctl_lpu237.read_card_from_device_with_callback(
        false,
        (_idx, _msg) => { /* ignore */ },
        (_idx, _err) => { /* ignore */ }
      );
    ...
    g_refresh_device_list = async () => {
      try {
        if (g_coffee.get_session_number()) {
          const dev_list = await g_coffee.get_device_list(MSR_DEVICE_FILTER,);
          const msrPaths = dev_list.filter(p => p.endsWith(MSR_PATH_SUFFIX));
          setState(prev => ({ ...prev, devicePaths: msrPaths }));
          addLog(`[Info] MSR device paths: ${msrPaths.length > 0 ? msrPaths.join(', ') : '(none)'}`);
  ...
  async function onDisconnectServer() {
    ...
    try {
      if (g_ctl_lpu237) {
        // 카드 읽기 대기 중이면 먼저 취소 후 close
        if (stateRef.current.isReading) {
          addLog('[Server] Stopping card read before disconnect...');
          ...
          addLog('[Server] Card read stopped.');
        }
  ...
  async function onDisconnect() {
    ...
    try {
      ...
      if (stateRef.current.isReading) {
        addLog('[Device] Stopping card read before disconnect...');
        await new Promise<void>((resolve) => {
          g_ctl_lpu237!.read_card_from_device_with_callback(
            false,
            (_idx, _msg) => { resolve(); },
        ...
        addLog('[Device] Card read stopped.');
      }
      // 10. close
  ...
  function onStartReading() {
    ...
    addLog('[MSR] Starting card read wait...');
    // 8. 카드 읽기 대기 상태로 전환
    g_ctl_lpu237.read_card_from_device_with_callback(
      true,
      _cb_read_msr_done,
  ...
  function onStopReading() {
    ...
    addLog('[MSR] Cancelling card read wait...');
    // 9. 카드 읽기 대기 취소
    g_ctl_lpu237.read_card_from_device_with_callback(
      false,
      _cb_stop_msr_done,
  ...
  function onClearHistory() {
    setState(prev => ({ ...prev, cardHistory: [] }));
    addLog('[Info] Card history cleared.');
  }
