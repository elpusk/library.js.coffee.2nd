...
const CardHistory: React.FC<CardHistoryProps> = ({ history, onClear }) => {
  return (
          ...
          {history.map((card, idx) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[10px] text-slate-400 font-mono">{card.timestamp}</span>
              </div>
              <div className="space-y-1">
                <TrackRow label="Track 1" value={card.track1} error={card.track1_error} />
                <TrackRow label="Track 2" value={card.track2} error={card.track2_error} />
                <TrackRow label="Track 3" value={card.track3} error={card.track3_error} />
              </div>
            </div>
          ))}
        </div>
