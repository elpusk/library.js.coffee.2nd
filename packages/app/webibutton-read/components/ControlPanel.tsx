  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4 space-y-4">
      ...
      <div className="space-y-1">
        <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">i-Button Device Path</label>
        <select
        ...
        >
          {devicePaths.length === 0 ? (
            <option value="">-- No i-BUtton device found --</option>
          ) : (
            devicePaths.map(p => (
      ...
      <div className="flex gap-2 pt-1">
        ...
        >
          <CreditCard size={14} />
          {isReading ? 'Waiting for ibutton...' : 'Start Reading'}
        </button>
        <button
      ...
      {isReading && (
        <div className="flex items-center gap-2 text-green-700 bg-green-50 border border-green-200 rounded px-3 py-2 text-xs font-semibold animate-pulse">
          <CreditCard size={14} />
          Waiting for contacting ibutton...
        </div>
      )}
    </div>
  );
